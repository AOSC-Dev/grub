...
grub_extcmd_t cmd;
grub_size_t grub_avail_mem_bytes = 0;
...
static int
memsize_hook (grub_size_t addr __attribute__ ((unused)),
              grub_size_t size, grub_memory_type_t type, void *data)
{
  /*
   * Iterate through GRUB memory map.
   * FIXME: Which type of memory region should we consider to add up?
   */
  grub_size_t *total = (grub_size_t *) data;
                  ...
                  char **args __attribute__ ((unused)))
{
  ...
  grub_size_t avail_mem = 0;
  /* The converted value. */
  grub_size_t converted_avail_mem = 0;
  ...
  if (converted_avail_mem > GRUB_INT_MAX) {
    return grub_error (GRUB_ERR_BAD_NUMBER,
      N_("Value is too large (%"
        PRIuGRUB_SIZE " > %" PRIdGRUB_INT32_T ")"),
